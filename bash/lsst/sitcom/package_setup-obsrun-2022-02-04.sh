...
git fetch --all
git checkout tickets/DM-33575
git reset origin/tickets/DM-33575 --hard
git pull
setup -j -r .

printf '\nSetting up lsst-ts/ts_config_attcs \n'
cd $REPOS
git clone https://github.com/lsst-ts/ts_config_attcs.git
cd ts_config_attcs
git fetch --all
git checkout tickets/DM-33466
git reset origin/tickets/DM-33466 --hard
git pull
setup -j -r .
