...
# This script is ONLY to be used for the AuxTel run of 2021-03-22
source ${LOADSTACK}

# Verify the proper jupyter build is being used
REQUIRED_BUILD='lsst-scipipe-0.4.3'
...
git fetch --all
git checkout tickets/DM-29197
git reset origin/tickets/DM-29197 --hard
git pull
setup -j -r .
